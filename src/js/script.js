...
let startBtn = document.getElementById('start'),
    ...
    inputOptIncome = document.querySelector('.choose-income'),
    accumulationCheck = document.querySelector('#savings'),
...
expensesBtn.addEventListener('click', function() {
    ...
    for (let i = 0; i < mustExpenses.length; i++) {
        let a = mustExpenses[i].value,
            b = mustExpenses[++i].value;
...
optionalExpensesBtn.addEventListener('click', function() {
    for (let i = 0; i < inputOptExpenses.length; i++) {
        let questionOptExpenses = inputOptExpenses[i].value;
...
inputOptIncome.addEventListener('input', function() {

    let items = inputOptIncome.value;
    // if ((typeof(items)) === 'string'  && items != null) { 
        appData.income = items.split(', ');
        optIncome.textContent = appData.income;
        
    // } else {
    //     optIncome.textContent = "Неверно введены данные";
    // }


});

accumulationCheck.addEventListener('click', function() {
